...
(function () {
    ...
    const fnCheckIfExists = (path, successFunction, errorFunction) => {
        if (!path) return;
        appFirebase.database().ref(path).once("value").then(successFunction, errorFunction);
    };
